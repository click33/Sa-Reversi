...
<template>
    <el-card class="user-info-box zdy-card fade-in-ys"
             :class=" gameStore.currentPlayerType === 'black' ? 'active-zhi-zi' : '' " shadow="never">
        <template #header>
            <span>黑棋玩家信息</span>
        ...
        <template #default>
            ...
            <!-- user 模式 -->
            <template v-if="selectStore.blackRole === 'user' ">
                <span class="user-name">{{ selectStore.playerName }}</span>
                <span class="user-sub-info">（玩家）</span>
            </template>
            <!-- AI 模式 -->
            <template v-else>
                <span class="user-name">{{ dictStore.getAIRole( selectStore.blackRole ).name }}</span>
